...
PORT = int(os.environ.get('PORT', 10000))  # ✅ استخدام المنفذ الصحيح
...
def get_user(user_id):
    ...
    if user:
        return {
            ...
            'last_name': user[3], 'balance': user[4], 'referrals_count': user[5],
            'referrer_id': user[6], 'vip_level': user[7], 'vip_expiry': user[8],
            'games_played_today': user[9], 'total_games_played': user[10],
            'total_earned': user[11], 'total_deposits': user[12],
            'games_counter': user[13], 'last_daily_bonus': user[14],
...
def save_user(user_data):
    ...
    ''', (
        ...
        user_data.get('last_name'), user_data.get('balance', 0.0),
        user_data.get('referrals_count', 0), user_data.get('referrer_id'),
        user_data.get('vip_level', 0), user_data.get('vip_expiry'),
        user_data.get('games_played_today', 0), user_data.get('total_games_played', 0),
        user_data.get('total_earned', 0.0), user_data.get('total_deposits', 0.0),
        user_data.get('games_counter', 0), user_data.get('last_daily_bonus')
    ))
...
def play_football_game(user_id):
    ...
    return random.choices(outcomes, k=3)

def play_basketball_game(user_id):
    return ["🎯 تسجيل ✅" if random.random() > 0.3 else "🎯 أخطأت ❌" for _ in range(3)]

def play_darts_game(user_id):
    return [f"🎯 نقاط: {random.randint(10, 50)}" for _ in range(3)]
...
def start_command(message):
    ...
    if not user:
        ...
        welcome_text = f"""🎮 أهلاً وسهلاً {message.from_user.first_name}!
...
🏆 اربح 5 USDT كل 3 محاولات!"""
        ...
        welcome_text = f"""🎮 مرحباً بعودتك {message.from_user.first_name}!
...
def handle_callbacks(call):
    ...
    if not user:
        bot.answer_callback_query(call.id, "❌ لم يتم العثور على بياناتك. أرسل /start")
        return
    ...
    try:
        if call.data == "main_menu":
            ...
            games_text = f"""🎮 **قائمة الألعاب المتاحة**
...
🚀 **بعد الشراء، سيتم التحقق من الإيداع تلقائياً!**"""
            ...
            confirmation_text = f"""🎖️ **تأكيد شراء {vip_info['name']}**
...
**استمتع بالمزايا الحصرية! 🏆**"""
            ...
            if user['balance'] < 100.0:
                bot.answer_callback_query(call.id, f"❌ الحد الأدنى للسحب 100 USDT! رصيدك: {user['balance']} USDT")
                return
            
            if user['total_deposits'] < 10.0:
                bot.answer_callback_query(call.id, f"❌ يجب أن تكون قد أودعت 10 USDT على الأقل للسحب! إيداعاتك: {user['total_deposits']} USDT")
                return
            
            withdraw_text = f"""💰 **طلب سحب رصيد**
...
⏰ **مدة المعالجة:** 24-48 ساعة"""
            ...
            confirmation_text = """✅ **تم تأكيد فهمك لشروط الأمان**
...
def get_vip_details(level):
    vip_data = {
        "bronze": {
            ...
            "daily_bonus": "0.5 USDT"
        },
        "silver": {
            "name": "🔵 VIP فضى", 
            "price": 10.0,
            "mining_bonus": "+25% أرباح تعدين",
            "daily_bonus": "1.0 USDT"
        },
        "gold": {
            "name": "🟡 VIP ذهبي",
            "price": 20.0,
            "mining_bonus": "+50% أرباح تعدين", 
            "daily_bonus": "2.0 USDT"
        }
    }
